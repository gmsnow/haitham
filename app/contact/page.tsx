...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!executeRecaptcha) {
      toast.error(dictionary.contact.notReady);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(data?.error ?? dictionary.contact.error);
        return;
      }
    ...
    >
      <div className="flex flex-col md:flex-row gap-6 w-full">
        <div className="flex-1 min-w-0">
          ...
          />
          {fieldErrors.name && (
            <p id="name-error" className="mt-1 text-start text-sm text-accent" role="alert">
              {dictionary.contact.invalidName}
            </p>
          )}
        </div>
        <div className="flex-1 min-w-0">
          ...
          />
          {fieldErrors.email && (
            <p id="email-error" className="mt-1 text-start text-sm text-accent" role="alert">
              {dictionary.contact.invalidEmail}
            </p>
          )}
        </div>
      </div>
      <div>
        ...
        />
        {fieldErrors.subject && (
          <p id="subject-error" className="mt-1 text-start text-sm text-accent" role="alert">
            {dictionary.contact.invalidSubject}
          </p>
        )}
      </div>
      <div>
        ...
        />
        {fieldErrors.message && (
          <p id="message-error" className="mt-1 text-start text-sm text-accent" role="alert">
            {dictionary.contact.invalidMessage}
          </p>
        )}
      </div>
      <button
      ...
      >
        ...
        >
          {isLoading ? dictionary.contact.sending : dictionary.contact.submit}
        </span>
...
const Contact = () => {
  ...
  return (
    <div className="h-full bg-primary/30 overflow-y-auto">
      <div className="container mx-auto py-24 sm:py-32 text-center xl:text-start flex items-center justify-center h-full">
        <div className="flex flex-col w-full max-w-175">
          ...
          {siteKey ? (
            ...
            <p className="text-center">{dictionary.contact.unavailable}</p>
          )}
        </div>
      </div>
    </div>
